...
function getAchievementRecords() {
    ...
    Object.keys(history).forEach(catId => {
        const catHistory = history[catId];
        if (!catHistory || typeof catHistory !== "object") return;

        Object.keys(catHistory).forEach(date => {
            const day = catHistory[date];
...
function getCompletedRecords(records) {
    return records.filter(record =>
        record.tasks.some(task => task.done === true)
    );
}
...
function getPlayDates(records) {
    return [...new Set(
        records
            .filter(record =>
                record.tasks.some(task =>
                    task.id === "play" && task.done === true
                )
            )
...
function getMonthlyCareAchievement(records) {
    const byMonth = {};
    ...
    records.forEach(record => {
        ...
        record.tasks.forEach(task => {
            monthData.total++;
            if (task.done === true) monthData.done++;
        });
    });

    return Object.values(byMonth).some(month =>
        month.days.size >= 7 &&
        month.total > 0 &&
        month.done / month.total >= 0.8
    );
}


function isProfileComplete(cat) {
    if (!cat || !cat.name) return false;

    const hasAge =
        Boolean(cat.birthDate) ||
        (
            cat.ageValue !== null &&
...
function hasIndividualCareForDifferentCats(records) {
    const completedByCat = {};

    records.forEach(record => {
        const individualDone = record.tasks.some(task =>
            task.done === true && task.shared !== true
        );

        if (individualDone) {
            completedByCat[record.catId] = true;
        }
    });

    return Object.keys(completedByCat).length >= 2;
}


function getFirstHistoryDate(records) {
    const dates = getUniqueDates(records);
    return dates[0] || null;
}


function hasYearOfCare(records) {
    const firstDate = getFirstHistoryDate(records);
...
function getAchievementStates() {
    ...
    const completedDates = getUniqueDates(completedRecords);
    const playDates = getPlayDates(records);
    const currentMonth = getTodayKey().slice(0, 7);
    const currentMonthPlayDates = playDates.filter(
        date => date.startsWith(currentMonth)
    );
    const playWeeks = new Set(
        playDates.map(getIsoWeekKey)
    );
    ...
    return [
        {
            icon: "🐾",
            title: "Первый шаг",
            text: "Выполнена первая задача.",
            unlocked: completedRecords.length > 0
        },
        {
            icon: "🐈",
            title: "Познакомились",
            text: "Заполнен профиль кошки.",
            unlocked: cats.some(isProfileComplete)
        },
        {
            icon: "📅",
            title: "Вместе неделю",
            text: "7 дней с выполненными задачами.",
            unlocked: completedDates.length >= 7
        },
        {
            icon: "🌿",
            title: "Хорошая привычка",
            text: "Задачи выполнялись 5 дней подряд.",
            unlocked: getConsecutiveDays(completedDates) >= 5
        },
        {
            icon: "🤍",
            title: "Заботливый месяц",
            text: "В одном месяце выполнено не менее 80% задач.",
            unlocked: getMonthlyCareAchievement(records)
        },
        {
            icon: "🎾",
            title: "Первая игра",
            text: "Впервые отмечена игра с кошкой.",
            unlocked: playDates.length >= 1
        },
        {
            icon: "🎾",
            title: "Любимая игра",
            text: "10 отмеченных игровых дней.",
            unlocked: playDates.length >= 10
        },
        {
            icon: "🐾",
            title: "Играем регулярно",
            text: "7 игровых дней за месяц.",
            unlocked: currentMonthPlayDates.length >= 7
        },
        {
            icon: "🗓️",
            title: "Вместе больше месяца",
            text: "Игровая активность отмечалась в 4 разные недели.",
            unlocked: playWeeks.size >= 4
        },
        {
            icon: "📖",
            title: "Начали наблюдать",
            text: "Первая запись в дневнике.",
            unlocked: records.length >= 1
        },
        {
            icon: "📚",
            title: "Наблюдатель",
            text: "10 записей в дневнике.",
            unlocked: records.length >= 10
        },
        {
            icon: "🐈‍⬛",
            title: "Большая семья",
            text: "Добавлены 2 кошки.",
            unlocked: cats.length >= 2
        },
        {
            icon: "🐾",
            title: "Каждому своё",
            text: "Индивидуальные задачи выполняются для разных кошек.",
            unlocked: cats.length >= 2 && hasIndividualCareForDifferentCats(records)
        },
        {
            icon: "⭐",
            title: "Год заботы",
            text: "Приложением пользуются уже год.",
            unlocked: hasYearOfCare(records)
        }
    ];
}
...
function createAchievementsSection() {
    ...
    return `
        <div class="section-title achievements-section-title">
            Достижения
        </div>
        ...
        <div class="card achievements-card">
            <div class="achievements-header">
                <div>
                    <strong>Маленькие отметки заботы</strong>
                    <span>Достижения открываются ретроспективно — без серий и штрафов.</span>
                </div>
                <b>${unlocked}/${achievements.length}</b>
            </div>
